...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        """
        logger.info(
            "Making call to API Member Rivalo\n"
        ...
        if (campaign_title == "rivalo col"):
            rivalo_channel_id = settings.API_ACCOUNT_MEMBER_REPORT_RIVALO_COL_CHANNEL_ID
            rivalo_client_id = settings.API_ACCOUNT_MEMBER_REPORT_RIVALO_COL_CLIENT_ID
            rivalo_client_secret = settings.API_ACCOUNT_MEMBER_REPORT_RIVALO_COL_CLIENT_SECRET
            rivalo_rs_percentage = settings.API_ACCOUNT_MEMBER_REPORT_RIVALO_COL_RS_PERCENTAGE
        else:
        ...
        if (
            not all(
                (
                    rivalo_channel_id,
                    rivalo_client_id,
                    rivalo_client_secret,
                    rivalo_rs_percentage,
                )
            )
        ):
            logger.error(
                f"Campaign with title \"{campaign_title}\" have a undefined settings vars, interpreted vars\n"
                f"rivalo_channel_id -> \"{rivalo_channel_id}\"\n"
                f"rivalo_client_id -> \"{rivalo_client_id}\"\n"
                f"rivalo_client_secret -> \"{rivalo_client_secret}\"\n"
                f"rivalo_rs_percentage -> \"{rivalo_rs_percentage}\""
            )
            return

        # Get OAuth2
        url = "https://login.rivalopartners.com/oauth/access_token"
        body = {
            "client_id": rivalo_client_id,
            "client_secret": rivalo_client_secret,
        ...
        # ---- Member report case ----
        # https://login.rivalopartners.com/statistics.php?p=3&d1=2022-07-01&d2=2022-07-11&cg=&c=&m=&o=&s=&pr=&sd=1&sc=1&mode=csv&sbm=1&dnl=1
        url = (
            f"https://login.rivalopartners.com/statistics.php?p={rivalo_channel_id}&"
        ...
        df = pd.read_csv(
            ...
            dtype={
                "Campaign": "string",
                "Total Deposit Amount": np.float32,
                "Signups": np.uint32,
                "First Deposit Count": np.uint32,
                "Net Rev Casino": np.float32,
                "Net Rev Sportsbook": np.float32,
                "Net Rev Virtual Sports": np.float32,
                "Income": np.float32,
        ...
        df.rename(
            ...
            columns={
                "Campaign": "prom_code",
                "Total Deposit Amount": "deposit",
                "Signups": "registered_count",
                "First Deposit Count": "first_deposit_count",
                "Net Rev Casino": "net_revenue_casino",
                "Net Rev Sportsbook": "net_revenue_sport",
                "Net Rev Virtual Sports": "net_revenue_virtual_sport",
        ...
        df.drop(
            labels=df[
                df.eval(
                    ...
                    "(net_revenue_casino == 0) &"
                    "(net_revenue_sport == 0) &"
                    "(net_revenue_virtual_sport == 0) ",
                    # "(cpa_count == 0)",
                    engine="numexpr",
                )
            ].index,
        ...
        for row in tqdm(zip(*df.to_dict("list").values())):
            ...
            if(update_month):
                betenlace_cpa = self.betenlace_month_update(
                    ...
                    campaign=campaign,
                    rs_percentage=rivalo_rs_percentage,
                )
    ...
    def betenlace_daily_create(
        ...
        net_revenue = (
            row[keys.get("net_revenue_sport")] +
            row[keys.get("net_revenue_casino")] +
            row[keys.get("net_revenue_virtual_sport")]
        )
