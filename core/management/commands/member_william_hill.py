...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "-c",
            "--campaign",
            default="william hill esp",
            choices=["william hill esp"],
    ...
    def handle(self, *args, **options):
        """
            Get data from API of bookmaker William Hill ESP with CSV files using 
        ...
        if (campaign_title == "william hill esp"):
            william_hill_key = settings.API_MEMBER_REPORT_WILLIAMHILLESP_KEY
            william_hill_account_id = settings.API_MEMBER_REPORT_WILLIAMHILLESP_ACCOUNT_ID
            william_hill_account_name = settings.API_MEMBER_REPORT_WILLIAMHILLESP_ACCOUNT_NAME

        try:
            url = (
                f"https://partners.williamhill.com/api/affreporting.asp?key={william_hill_key}"
                f"&reportname=Member%20Report%20-%20Detailed&reportformat=csv"
                f"&reportmerchantid={william_hill_account_id}&reportstartdate={from_date_str}"
        ...
        # "purchaccountcount","wageraccountcount","avgactivedays","purchases",
        # "netbets","netwagers","netpoints","prod1bets","prod1wagers",
        # "prod1netrevenue","prod1commission","prod2bets","prod2wagers",
        # "prod2netrevenue","prod2commission","prod3bets","prod3wagers",
        # "prod3netrevenue","prod3commission","prod4bets","prod4wagers",
        # "prod4netrevenue","prod4commission","prod5bets","prod5wagers",
        # "prod5netrevenue","prod5commission","prod6bets","prod6wagers",
        # "prod6netrevenue","prod6commission","prod7bets","prod7wagers",
        # "prod7netrevenue","prod7commission","prod8bets","prod8wagers",
        # "prod8netrevenue","prod8commission","prod9bets","prod9wagers",
        # "prod9netrevenue","prod9commission","prod10bets","prod10wagers",
        # "prod10netrevenue","prod10commission","revsharecommission",
        # "totalcpacommission","cpacommissioncount","referralcommissiontotal",
        # "totalcommission"
        ...
        df.drop(
            labels=df[
                df.eval(
                    "(row_id == 2) |"
                    f"(account_name != '{william_hill_account_name}')",
                    engine="numexpr",
                )
            ].index,
            inplace=True,
        ...
        # Set dataframe with cols for iteration
        cols_to_use = [
            "prom_code",
            "deposit",
            "stake",
