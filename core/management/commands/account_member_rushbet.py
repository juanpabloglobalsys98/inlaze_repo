...
class Command(BaseCommand):
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "-fa", "--file_account", nargs='?',
            help=('name of file where storage the csv, if supplied the DB '
                  'statement will not executed'))
        parser.add_argument(
            "-fm", "--file_member", nargs='?',
            help=('name of file where storage the csv, if supplied the DB '
                  'statement will not executed'))
    ...
    def handle(self, *args, **options):
        ...
        """
        logger.info(
            "Making call to API Account Rushbet\n"
            f"Campaign Title -> {options.get('campaign')}\n"
            f"From date -> {options.get('fromdate')}\n"
            f"To date -> {options.get('todate')}\n"
            f"File to save Account -> {options.get('file_account')}\n"
            f"File to save Member -> {options.get('file_member')}"
        )
        ...
        # with open("rushbet_account_bas.csv", "w") as out:
        ...
        # "registrationdate","%Commission","totalcommission","new"

        if(options.get("file_account")):
            # File case save to disk and prevent execute on DB
            df_account.to_csv(
                path_or_buf=options.get("file_account"),
                index=False,
                encoding="utf-8",
            )
            return
        ...
        # "wageraccountcount","avgactivedays","revsharecommission",
        # "totalcommission"

        if(options.get("file_member")):
            # File case save to disk and prevent execute on DB
            df_member.to_csv(
                path_or_buf=options.get("file_member"),
                index=False,
                encoding="utf-8",
            )
            return
        ...
        for row in tqdm(zip(*df_account.to_dict('list').values()), desc="account"):
            """
            - row_id
            - prom_code
            - punter_id
        ...
        for row in tqdm(zip(*df_member.to_dict('list').values()), desc="member"):
