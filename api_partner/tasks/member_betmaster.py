...
def member_betmaster(campaign_title):
    """
    ...
    if (
        ...
        logger_msg = f"Campaign with title \"{campaign_title}\" undefined settings vars"
        logger_task.error(logger_msg)
        logger_msg = f"*LEVEL:* `ERROR` \n*message:* `{logger_msg}`\n\n"
        chat_logger_task.apply_async(
            kwargs={
    ...
    for row in zip(*df.to_dict('list').values()):
        ...
        try:
            ...
            logger_msg = (
                f"Betenlace CPA entry not found for campaign=\"{campaign_title}\" with "
    ...
    if (len(df.index) == 0):
        ...
        logger_msg = f"Member for Campaign {campaign_title} processed count {len(df.index)}"
        logger_task.warning(logger_msg)
        logger_msg = f"*LEVEL:* `WARNING` \n*message:* `{logger_msg}`\n\n"
