...
def member_betsson(campaign_title):
    """
    ...
    """
    # Get latest billed
    latest_withdrawal_billed_to_at = WithdrawalPartnerMoney.objects.all().only(
        "billed_to_at",
    ).order_by("-billed_to_at").first()
    if (latest_withdrawal_billed_to_at is None):
        latest_billed_at = timezone.now().astimezone(pytz.timezone(settings.TIME_ZONE)) - timedelta(weeks=5)
        logger_msg = (
            f"WithdrawalPartnerMoney not found using default date {latest_billed_at}"
        )
        logger_task.warning(logger_msg)
        logger_msg = f"*LEVEL:* `WARNING` \n*message:* `{logger_msg}`\n\n"
        ...
        latest_billed_at = latest_withdrawal_billed_to_at.billed_to_at

    for days_count in range(5, 0, -1):
        # Definition of function
        today = timezone.now().astimezone(pytz.timezone(settings.TIME_ZONE))
        yesterday = today - timedelta(days=days_count)
        ...
        logger_task.info(logger_msg)
        logger_msg = f"*LEVEL:* `INFO` \n*message:* `{logger_msg}`\n\n"
        ...
        if (latest_billed_at >= yesterday.date()):
            logger_msg = (
                f"Date {yesterday} is already billed, prevented get data"
            )
            logger_task.warning(logger_msg)
            logger_msg = f"*LEVEL:* `WARNING` \n*message:* `{logger_msg}`\n\n"
            chat_logger_task.apply_async(
...
def _member_report(
    ...
    if not campaign:
        logger_msg = f"Campaign with title \"{campaign_title}\" not found in DB"
        logger_task.error(logger_msg)
        logger_msg = f"*LEVEL:* `ERROR` \n*message:* `{logger_msg}`\n\n"
        chat_logger_task.apply_async(
            kwargs={
                "msg": logger_msg,
                "msg_url": settings.CHAT_WEBHOOK_CELERY,
    ...
    if (campaign_title == "betsson col"):
        ...
        logger_msg = (
            f"Campaign with title {campaign_title} not defined on account member betsson"
        )
        logger_task.error(logger_msg)
        logger_msg = f"*LEVEL:* `ERROR` \n*message:* `{logger_msg}`\n\n"
        chat_logger_task.apply_async(
    ...
    # Check already data today
    is_data_today = bool(
        BetenlaceDailyReport.objects.filter(
            (
                Q(deposit__gt=0) |
                Q(stake__gt=0) |
                Q(net_revenue__gt=0) |
                Q(revenue_share__gt=0) |
                Q(fixed_income__gt=0) |
                Q(registered_count__gt=0) |
                Q(first_deposit_count__gt=0) |
                Q(wagering_count__gt=0)
            ) &
            Q(betenlace_cpa__link__campaign_id=campaign.pk) &
    ...
    if (is_data_today):
        logger_msg = (
            f"Already exist for campaign_title \"{campaign_title}\" data at \"{yesterday_str}\", stoped"
        )
        logger_task.warning(logger_msg)
        logger_msg = f"*LEVEL:* `WARNING` \n*message:* `{logger_msg}`\n\n"
        return
    ...
    url = (
        ...
        f"d1={yesterday_str}&d2={yesterday_str}&cg=&c=&m=&o=&s=&sd=1&sc=1&mode=csv&sbm=1&dnl=1"
    )
    ...
    if(fx_partner is None):
        logger_msg = ("Undefined fx_partner on DB")
        logger_task.error(logger_msg)
        logger_msg = f"*LEVEL:* `ERROR` \n*message:* `{logger_msg}`\n\n"
        chat_logger_task.apply_async(
            kwargs={
    ...
    if (len(df_member.index) == 0):
        logger_msg = f"Member for Campaign {campaign_title} No Records/No data"
        logger_task.warning(logger_msg)
        logger_msg = f"*LEVEL:* `WARNING` \n*message:* `{logger_msg}`\n\n"
        ...
        logger_msg = f"Member for Campaign {campaign_title} processed count {len(df_member.index)}"
        logger_task.warning(logger_msg)
        logger_msg = f"*LEVEL:* `WARNING` \n*message:* `{logger_msg}`\n\n"
